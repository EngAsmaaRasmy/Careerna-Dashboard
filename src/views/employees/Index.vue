...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url: process.env.VUE_APP_BACKEND_SERVER + 'admin-dashboard/employees',
          dataSrc: (json) => {
            return json.data
          }
        },
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        process.env.VUE_APP_BACKEND_SERVER + 'admin-dashboard/employees',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}admin-dashboard/employees/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {
