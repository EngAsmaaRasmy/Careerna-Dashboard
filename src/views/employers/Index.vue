...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url: process.env.VUE_APP_BACKEND_SERVER + 'admin-dashboard/employers',
          dataSrc: (json) => {
            return json.data
          }
        },
      ...
      fields: {
        ...
        actions: {
          ...
          defaultContent: '<button href="javascript:void(0);" data-action="show" class="btn btn-info btn-lg" title="عرض الطلب"><i class="mdi mdi-square-show-outline"></i>' + this.$t('message.show') + ' </button>' +
            '&ensp;'
        }
      },
      quickSearch: '',
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        process.env.VUE_APP_BACKEND_SERVER + 'admin-dashboard/employers',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertShow (data) {
      this.$router.push({ path: `employers/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/employers/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}admin-dashboard/employers/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {
